...
declare % Creación del generador de números enteros
fun {Producer Start End}
   if Start<End then Start|{Producer Start+1 End}
   else nil end
end
fun {Consumer Xs A} % Consumidor o cálculo de la suma de números impares
   case Xs of X|Xr then {Consumer Xr A+X}
   [] nil then A end
end
fun {IsOdd X} % Filtro para números impares
   X mod 2 \= 0
end
